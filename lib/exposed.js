...
Exposed.create = function (exposed) {
    ...
    // serialized cached state, aggregation of appliciable namespaces happens at
    // `toString()` time.
...
Exposed.prototype._getApplicableNamespaces = function () {
    ...
    //
    // This dedupping keeps the set of exposed values as small as possible by
    ...
    // namespaces which are logically appliciable, ordered by: grandparent,
    // parent, then child/this. Each instance's namespaces will already be
    // ordered and logically dedupped by every `add()` call.
